...
function doGetObjects(){
	...
	for(var i in result){
		var tr=$("<tr></tr>");
		tr.data("id",result[i].tid);
		tr.append('<td>'+result[i].time+'</td>');
		tr.append('<td>'+result[i].amount+'</td>');
		tr.append('<td>'+result[i].openId+'</td>');
		tr.append('<td>'+result[i].nickName+'</td>');
		tr.append('<td><img src='+result[i].avatarUrl+' width="30" height="30"/></td>');
